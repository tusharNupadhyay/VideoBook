...
const [videoToDelete, setVideoToDelete] = useState(null);
  ...
  if (myVideosLoading && !myVideos.videos.length) return <div className="p-10 text-white">Loading dashboard...</div>;
  if (myVideosError) return <div className="p-6 text-red-500 bg-red-500/10 rounded-lg m-4">{myVideosError}</div>;
    ...
    <div className="flex-1 bg-[#0f0f0f] text-white p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <p className="text-neutral-400 text-sm">Total: {myVideos?.total || 0} videos</p>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 px-4 py-3 bg-neutral-900 border-b border-neutral-800 text-neutral-400 text-xs font-semibold uppercase tracking-wider">
          ...
          <div className="col-span-3 md:col-span-3 text-right pr-4">Options</div>
        </div>

        {myVideos?.videos?.length === 0 ? (
          <div className="text-center py-20 text-neutral-500">You haven't uploaded any videos yet.</div>
        ) : (
